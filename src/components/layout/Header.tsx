          ...
          <div className="flex items-center justify-between h-20 lg:h-[84px]">
            ...
            <div className={`hidden lg:flex items-center gap-4 lg:flex-none justify-end ${textClass}`}>
              ...
              {!loading && user ? (
                <Link
                  href="/account"
                  className="flex items-center gap-2 p-1.5 transition-opacity duration-300 hover:opacity-50"
                  aria-label="Account"
                >
                  <div className="w-[18px] h-[18px] rounded-full bg-walnut text-cream flex items-center justify-center">
                    <span className="font-sans text-[0.5rem] font-semibold leading-none">
                      {getUserInitials()}
                    </span>
                  </div>
                  <span className="font-sans text-[0.6875rem] font-medium">
                    {getDisplayName()}
                  </span>
                </Link>
              ) : (
              ...
              <div className="px-6 py-5 border-t border-sand/30">
                {!loading && user ? (
                  <>
                    ...
                    >
                      <div className="w-8 h-8 rounded-full bg-walnut text-cream flex items-center justify-center">
                        <span className="font-sans text-[0.625rem] font-semibold leading-none">
                          {getUserInitials()}
                        </span>
                      </div>
                      <div className="flex flex-col">
                        <span className="font-medium">{getDisplayName()}</span>
                        <span className="text-[0.6875rem] text-walnut/60">{user.email}</span>
                      </div>
                    </Link>
                    <button
                      onClick={() => { signOut(); setMobileOpen(false); }}
                      className="flex items-center gap-3 py-3 font-sans text-[0.8125rem] font-medium text-walnut/60 hover:text-walnut transition-colors"
                    >
                      <svg
                        width="16"
                        height="16"
